...
public class ICabDaoImpl implements ICabDao {
	...
	public int countCabsOfType(String carType) throws CabNotFoundException {
		//int q = (int)em.createQuery("SELECT COUNT(c) FROM Cab c where carType = :n").setParameter("n", carType).getSingleResult();
		//TypedQuery<Cab> q = em.createQuery("select count(c) from Cab c where c.carType = :n", Cab.class);
		/*
		 * TypedQuery<Cab> q =
